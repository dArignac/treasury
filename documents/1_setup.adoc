...
To be able to use the ratings feature, a custom host name must be set up.
This has to be done as _Treasury_ will redirect the user to _The Movie DB_ to allow access on
her data. The redirection will not work if you run the application just under the default 
localhost host.

So add a custom host name into `/etc/hosts`, e.g. `treasury.local`: `127.0.0.1 treasury.local` 
and then access the local development environment using this host instead of `localhost`.
...
** add the custom host name, that was setup in one of the previous steps, to the allowed domains to allow login from there
